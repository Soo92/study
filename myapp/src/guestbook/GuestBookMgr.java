...
public class GuestBookMgr {
	...
	//Join Login
	public boolean loginJoin(String id, String pass) {
		Connection con = null;
		...
		try {
			...
			sql = "select * from tblJoin where id=? and pass=?";
	...
	//GuestBook Post
	public void postGuestBook(GuestBookBean bean) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
	...
	public Vector<GuestBookBean> listGuestBook(String id, String grade){
		...
		try {
			...
			if(grade.equals("1"/*admin*/)) {
				...
				sql = "select * from tblGuestBook where id=? or secret=? "
						+ "order by num desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, id);
				pstmt.setString(2, "0");
			...
			while(rs.next()) {
				...
				String tempDate = 
						SDF_DATE.format(rs.getDate(5));
				bean.setRegdate(tempDate);
				String tempTime = 
						SDF_TIME.format(rs.getTime(6));
	...
	//GuestBook Update
	public boolean updateGuestBook(GuestBookBean bean){
		//contents, ip, secret 3개만 수정
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		boolean flag = false;
		try {
			con = pool.getConnection();
			sql = "update tblGuestBook set contents=?,"
					+"ip=?,secret=? where num=?";
			pstmt = con.prepareStatement(sql);
	...
	//GuestBook Delete
	public boolean deleteGuestBook(int num){
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		boolean flag = false;
		try {
			con = pool.getConnection();
			sql = "delete from tblGuestBook where "
				   +"num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);	
			if(pstmt.executeUpdate()==1)
