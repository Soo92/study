...
public class RegisterMgr {
	...
	public boolean insertResiter(RegisterBean regBean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert tblRegister values(?,?,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, regBean.getId());
			pstmt.setString(2, regBean.getPass());
	...
	public int loginCheck(String id, String pass) {
		...
		try {
			if (!checkId(id))
				return mode;
			con = pool.getConnection();
			sql = "select id, pass from tblRegister where id=? and pass=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
	...
	public boolean updateRegister(RegisterBean regBean) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblRegister set pass=?,name=?," + "email=?,phone=?,zipcode=?,address=?,job=?,hobby=? "
					+ "where id = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, regBean.getPass());
